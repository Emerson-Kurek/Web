...
The sheet stays white and the body type stays near-black. One colour does all
the attention work: a red keyline around the sheet, a red rule box around the
discount, and the figures those boxes enclose set in the same red. A single
saturated mark in an otherwise neutral field is what the eye lands on first, and
it survives the distance at which detail has already collapsed.

Outline rather than fill is a deliberate trade. A solid block of colour shouts
slightly louder at 10 m, but it prints heavy and it reads cheap — the look of a
discount bin rather than a showroom running a sale. A rule box holds the same
territory, keeps the number clean and legible inside it, and costs almost no
ink. Given a store whose product is *good light*, looking considered is part of
the sell, so the poster buys attention with contrast and structure instead of
coverage. This keeps the white-ground rule from `PLINTH-001-prompt.md` intact.
...
| Token | Value | Job |
|---|---|---|
| `--ink` | `#17181a` | all body copy |
| `--red` | `#d81f06` | keylines, and the large figures inside them |
| `--red-deep` | `#b01604` | the same red for smaller type |
| `--accent` | `#f08018` | brand orange, now only in the logo roundel |

Two strengths of one red, for the same reason the orange had two: `#d81f06` on
white runs about 4.4:1, which is fine for a 200 mm numeral and marginal for a
9 mm line, so smaller type steps down to `#b01604` at about 6:1.

The brand orange survives only inside the logo. Orange and red next to each
other at poster scale muddle rather than reinforce, and the mark already carries
the brand — repeating its colour elsewhere would weaken the one red signal the
sheet depends on.

## Ink coverage

Light. The sheet is white, the coverage is rules and type, and there is no
flooded area anywhere on it. It prints economically at A1 and behaves on an
office printer in the Letter proof mode, which is a large part of why the
outline treatment was chosen over solid blocks.
